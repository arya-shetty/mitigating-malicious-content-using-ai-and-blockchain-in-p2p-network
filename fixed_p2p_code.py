...
HOST_IP = '172.20.10.3'        # <-- set your blockchain host IP here
...
def main():
    ...
    while True:
        ...
        if cmd.lower() == 'exit':
            ...
            print("[Blockchain] Requesting blockchain from host...")
            blockchain = request_blockchain_from_host()
            if blockchain:
                print(f"\n[Blockchain] Retrieved {len(blockchain)} blocks:")
                for i, block in enumerate(blockchain):
                    print(f"  Block {i}: {json.dumps(block, indent=2)}")
            else:
                print("[Blockchain] No blockchain data received or connection failed.")
        else:
